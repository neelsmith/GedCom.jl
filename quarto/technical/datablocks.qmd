...
Extract groups of records with the `GedCom.blocks` function.  A "block" is just a vector of `GEDRecord`s, so the result of this function is a vector of vectors of `GEDRecord`s.
